...
# Failed
execute if score $1.16 twvm.tool matches 0 run tellraw @a[tag=!global.ignore,tag=!global.ignore.gui] [{"text":"[Datapack]: ","color":"red","bold":true},{"text":"Auto Utilities: Vein Mining failed to be loaded. Either upgrade to Minecraft 1.16+ or use an older version of this data pack.","color":"white","bold":false}]
